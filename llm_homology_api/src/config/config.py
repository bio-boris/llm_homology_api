...
class LLMHomologyApiSettings(BaseSettings):
    ...
    MAX_RESIDUE_HEADER_LENGTH: int = (
        100  # The maximum length of the protein sequence header
    )
    MAX_RESIDUE_COUNT: int = (
        5000  # The maximum number of residues allowed in a single protein sequence
    )
    MAX_PROTEINS_PER_REQUEST: int = (
        500  # The maximum number of protein sequences allowed in a single request
    )
